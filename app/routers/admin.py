...
async def admin_dashboard(request: Request, admin: dict = Depends(admin_required)):
    ...
    # Set minimum values to avoid empty charts
    transaction_status = {
        "pending": max(pending_transactions_count, 0),
        "completed": max(completed_transactions, 0),
        "cancelled": max(cancelled_transactions, 0)
    }
    
    transaction_types = {
        "purchase": max(purchase_transactions, 0),
        "donation": max(donation_transactions, 0)
    }
    ...
    # Set minimum values to avoid empty charts
    campaign_status = {
        "active": max(active_campaigns, 0),
        "completed": max(completed_campaigns, 0), 
        "cancelled": max(cancelled_campaigns, 0)
    }
    
    campaign_types = {
        "fundraising": max(fundraising_campaigns, 0),
        "donation": max(donation_campaigns, 0),
        "exchange": max(exchange_campaigns, 0)
    }
...
async def list_users(
    ...
    return templates.TemplateResponse(
        "admin/users.html",
        {"request": request, "user": admin, "users": users, "page": page, 
         "total_pages": total_pages, "search": search, "role": role, "status": status}
    )
...
async def edit_user_form(
    ...
    if not user_to_edit:
        return templates.TemplateResponse(
            "admin/error.html",
            {"request": request, "user": admin, "message": "User not found"}
        )
    
    return templates.TemplateResponse(
        "admin/edit_user.html",
        {"request": request, "user": admin, "edit_user": user_to_edit}
    )
...
async def update_user(
    ...
    if result.modified_count:
        ...
        return templates.TemplateResponse(
            "admin/error.html",
            {"request": request, "user": admin, "message": "User update failed"}
        )
...
async def delete_user(
    ...
    transactions = await db.transactions.find_one({
        "$or": [
...
async def pending_items(
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "user": admin, "items": items, 
...
async def view_item(
    ...
    if not item:
        return templates.TemplateResponse(
            "admin/error.html",
            {"request": request, "user": admin, "message": "Item not found"}
        )
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "user": admin, "item": item, 
...
async def list_campaigns(
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "user": admin, "campaigns": campaigns, 
         "page": page, "total_pages": total_pages, "status": status}
    )
...
async def create_campaign(
    ...
    try:
        ...
        if not organizer:
            ...
            return templates.TemplateResponse(
                "admin/create_campaign.html",
                {
                    ...
                    "user": admin,
                    "users": users,
                    "error": "Selected organizer not found"
                }
            )
        ...
        if result.inserted_id:
            ...
            return templates.TemplateResponse(
                "admin/error.html",
                {"request": request, "user": admin, "message": "Failed to create campaign"}
            )
        ...
        return templates.TemplateResponse(
            "admin/error.html",
            {"request": request, "user": admin, "message": f"Error creating campaign: {str(e)}"}
        )
...
async def view_campaign(
    ...
    try:
        ...
        if not campaign:
            return templates.TemplateResponse(
                "admin/error.html",
                {"request": request, "user": admin, "message": "Campaign not found"}
            )
        ...
        return templates.TemplateResponse(
            ...
            {"request": request, "user": admin, "campaign": campaign, 
             "creator": creator, "campaign_items": campaign_items,
             "total_value": total_value, "now": datetime.now}
        )
    except Exception as e:
        print(f"Error viewing campaign: {str(e)}")
        return templates.TemplateResponse(
            "admin/error.html",
            {"request": request, "user": admin, "message": f"Error viewing campaign: {str(e)}"}
        )
...
async def edit_campaign(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "admin/error.html",
            {"request": request, "user": admin, "message": f"Error updating campaign: {str(e)}"}
        )
...
async def view_user_profile(
    ...
    }).sort("transaction_date", -1).to_list(length=100)
    
    # Get campaigns the user has participated in
    user_donations = await db.transactions.find({
        "buyer_user_id": user_id,
    ...
    if campaign_ids:
        participated_campaigns = await db.campaigns.find({
            "campaign_id": {"$in": campaign_ids}
        }).to_list(length=100)
    
    # Get campaigns created by the user
    created_campaigns = await db.campaigns.find({
        "created_by": user_id
    }).to_list(length=100)
